...
/*	Vector3D direction = {0, 1, 0};
	...
	if(collidingSpatialObject)
	{
		...
		Collider::resolveCollision(this->shape, VirtualList::front(this->shapes), otherColliderToRemove, displacement, true);
	}
	*/
}
...
void Hero::jump(bool checkIfYMovement)
{
	...
	allowedNumberOfJumps = 127;
		...
		if(this->jumps < allowedNumberOfJumps)
		{
			...
			extern SoundSpec JumpSound;
			SoundManager::playSound(SoundManager::getInstance(), &JumpSound, kPlayAll, (const Vector3D*)&this->transformation.globalPosition, kSoundPlaybackNormal, NULL, NULL);
		}
	}
}
...
void Hero::takeDamageFrom(SpatialObject collidingObject, int energyToReduce, bool pause, bool invincibleWins)
{
	...
	if(!Hero::isInvincible(this) || !invincibleWins)
	{
		...
		// play hit sound
		extern SoundSpec FireSound;
		SoundManager::playSound(SoundManager::getInstance(), &FireSound, kPlayAll, (const Vector3D*)&this->transformation.globalPosition, kSoundPlaybackNormal, NULL, NULL);
...
void Hero::enterDoor()
{
	...
	*/
	...
	// play door sound
	extern SoundSpec CollectSound;
	SoundManager::playSound(SoundManager::getInstance(), &CollectSound, kPlayAll, (const Vector3D*)&this->transformation.globalPosition, kSoundPlaybackNormal, NULL, NULL);
}
...
{
	ASSERT(collisionInformation->otherCollider, "Hero::enterCollision: null otherCollider");
	...
	switch(SpatialObject::getInGameType(collidingObject))
	{
		...
		case kTypeTopCollider:
			{
				if((0 > Body::getVelocity(this->body)->y) || Hero::isBelow(this, collisionInformation->collider, collisionInformation))
				{
...
bool Hero::isBelow(Collider shape, const CollisionInformation* collisionInformation __attribute__((unused)))
{
	return false;
/*	
	RightBox shapeRightBox = Collider::getSurroundingRightBox(shape);
	RightBox otherColliderRightBox = Collider::getSurroundingRightBox(collisionInformation->otherCollider);

	fix10_6 heroBottomPosition = shapeRightBox.y1 - ((shapeRightBox.y1 - shapeRightBox.y0) >> 1) - (Body::getLastDisplacement(this->body).y << 1) / 2;

	return heroBottomPosition > otherColliderRightBox.y0 || __ABS(collisionInformation->solutionVector.direction.y) < __ABS(collisionInformation->solutionVector.direction.x);
	*/
}
...
void Hero::exitCollision(Collider shape, Collider shapeNotCollidingAnymore, bool isColliderImpenetrable)
{
	SpatialObject nonCollidingSpatialObject = Collider::getOwner(shapeNotCollidingAnymore);
	...
	Base::exitCollision(this, shape, shapeNotCollidingAnymore, isColliderImpenetrable);
}
