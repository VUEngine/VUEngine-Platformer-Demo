...
void Collision::initialTransform(const Transformation* environmentTransform)
{
	...
	if(!this->shapes)
	{
		...
		Collider shape = CollisionManager::createCollider(VUEngine::getCollisionManager(VUEngine::getInstance()), SpatialObject::safeCast(this), &shapeSpec);
		...
		Collider::transform(shape, myPosition, myRotation, myScale, &this->size);
